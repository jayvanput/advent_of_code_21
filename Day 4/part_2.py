...
def find_all_winning_boards(order, list_of_boards):
    win_order = []
    completed_indexes = []
    ...
    for draw in order:
        for board_index, board in enumerate(list_of_boards):
            if board_index in completed_indexes:
                continue
            for index, row in enumerate(board):
            ...
            if check_for_win(board) is not None:
                ...
                completed_indexes.append(board_index)

            if len(completed_indexes) == 99:
                return board, draw
...
if __name__ == "__main__":
    ...
    losing_board, final_draw = find_all_winning_boards(
        draw_order, list_of_boards)

    for index, row in enumerate(losing_board):
        losing_board[index] = [x if x != 1000 else 0 for x in row]
